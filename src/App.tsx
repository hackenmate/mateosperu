...
import { createMercadoPagoCheckout, createOrder, loadPublicProducts } from './store';
...
export default function App(){
  ...
  useEffect(()=>{loadPublicProducts().then(setProducts).finally(()=>setLoading(false))},[]);
  ...
  const openProduct=(p:Product)=>{setSelected(p);setSize(p.sizes[0]||'Única');setColor(p.colors[0]||'Único')};
  const add=()=>{if(!selected)return;setCart(c=>{const i=c.findIndex(x=>x.id===selected.id&&x.size===size&&x.color===color);return i>=0?c.map((x,n)=>n===i?{...x,quantity:x.quantity+1}:x):[...c,{...selected,size,color,quantity:1}]});setSelected(null);setCartOpen(true)};
  ...
  const submitOnline=async()=>{
    ...
    }catch(e:any){setNotice(e?.message||'No se pudo iniciar el pago online.');setProcessing(false);}
  };
  ...
  return <div className="min-h-screen bg-[#f4f2ed] text-[#101010]">
    <header className="sticky top-0 z-40 border-b border-black/10 bg-[#f4f2ed]/95 backdrop-blur">
      <div className="bg-black px-4 py-2 text-center text-[11px] font-bold uppercase tracking-[.16em] text-white">Envíos desde Arequipa a todo el Perú · Compra segura online o por WhatsApp</div>
    ...
    {selected&&<div className="fixed inset-0 z-50 grid place-items-center bg-black/65 p-4"><div className="grid max-h-[94vh] w-full max-w-4xl overflow-y-auto bg-[#f4f2ed] md:grid-cols-2"><img src={selected.image||selected.images[0]} className="h-full max-h-[650px] w-full object-cover"/><div className="relative p-7"><button className="absolute right-5 top-5" onClick={()=>setSelected(null)}><X/></button><p className="text-xs font-bold uppercase text-black/45">{selected.category} · {selected.collection}</p><h3 className="mt-2 font-display text-5xl">{selected.name}</h3><p className="mt-3 text-2xl font-black">{money(selected.price)}</p><p className="mt-4 text-sm text-black/60">{selected.description}</p><p className="mt-6 text-xs font-black uppercase">Talla</p><div className="mt-2 flex flex-wrap gap-2">{selected.sizes.map(s=><button key={s} onClick={()=>setSize(s)} className={`border px-4 py-2 ${size===s?'bg-black text-white':'bg-white'}`}>{s}</button>)}</div><p className="mt-6 text-xs font-black uppercase">Color</p><div className="mt-2 flex flex-wrap gap-2">{selected.colors.map(c=><button key={c} onClick={()=>setColor(c)} className={`border px-4 py-2 ${color===c?'bg-black text-white':'bg-white'}`}>{c}</button>)}</div><button onClick={add} disabled={selected.stock<=0} className="mt-7 w-full bg-black px-5 py-4 font-black uppercase text-white disabled:opacity-30">Agregar al carrito</button></div></div></div>}
    ...
    <AdminPanel open={admin} onClose={()=>setAdmin(false)} onCatalogChanged={()=>loadPublicProducts().then(setProducts)}/>
  </div>;
}
