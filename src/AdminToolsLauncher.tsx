...
import { BarChart3, Bell, Boxes, Camera, Check, Link2, Save, Settings2, ShieldCheck, UserPlus, X } from 'lucide-react';
...
type Tab='stock'|'photos'|'analytics'|'social'|'requests';
...
export default function AdminToolsLauncher(){
  ...
  useEffect(()=>{
    ...
    const unsub=onAuthChange(()=>void refreshIdentity());
    return unsub;
  },[]);
  ...
  const filtered=useMemo(()=>{
    ...
    return products.filter(p=>!q||`${p.name} ${p.category} ${p.collection}`.toLowerCase().includes(q));
  },[products,query]);
  ...
  const setVariantStock=(productId:string,variantId:string,value:number)=>setProducts(current=>current.map(p=>p.id!==productId?p:{...p,variants:(p.variants||[]).map(v=>v.id===variantId?{...v,stock:Math.max(0,value||0)}:v)}));
  const saveStock=async(product:Product)=>{setBusy(product.id);setMsg('');try{await saveProduct(product);await refreshAdmin();setMsg(`Stock de “${product.name}” actualizado.`)}catch(e:any){setMsg(e?.message||'No se pudo guardar el stock.')}finally{setBusy('')}};
  ...
  if(!isAdmin){return <>
    <button onClick={()=>setOpen(true)} className="fixed bottom-5 left-5 z-[76] rounded-full border border-black bg-white px-4 py-3 text-xs font-black uppercase shadow-xl"><UserPlus size={17} className="inline mr-2"/>Solicitar admin</button>
  ...
  return <>
    ...
    {open&&<div className="fixed inset-0 z-[140] overflow-y-auto bg-black/75 p-3 sm:p-5"><div className="mx-auto my-4 max-w-6xl bg-[#f4f2ed] p-4 sm:p-6"><div className="flex items-start justify-between gap-4"><div><p className="text-xs font-black uppercase tracking-[.18em] text-black/40">Mateo’s</p><h2 className="font-display text-4xl sm:text-5xl">CENTRO ADMIN</h2></div><button onClick={()=>setOpen(false)}><X/></button></div>
      <div className="mt-5 flex gap-2 overflow-x-auto pb-2">{([['stock','Stock',Boxes],['photos','Fotos por color',Camera],['analytics','Analítica',BarChart3],['social','Redes',Link2],...(isOwner?[['requests','Solicitudes',ShieldCheck] as const]:[])] as any[]).map(([id,label,Icon])=><button key={id} onClick={()=>setTab(id)} className={`flex shrink-0 items-center gap-2 px-4 py-3 text-xs font-black uppercase ${tab===id?'bg-black text-white':'border border-black/20 bg-white'}`}><Icon size={16}/>{label}</button>)}</div>
      {msg&&<div className="mt-4 bg-white p-3 text-sm font-bold">{msg}</div>}

      {tab==='stock'&&<div className="mt-5"><input className="field" placeholder="Buscar producto" value={query} onChange={e=>setQuery(e.target.value)}/><div className="mt-4 space-y-4">{filtered.map(product=><section key={product.id} className="bg-white p-4"><div className="flex flex-wrap justify-between gap-2"><div><b>{product.name}</b><p className="text-xs text-black/50">{product.category} · {product.active?'Publicado':'Borrador'}</p></div><b>Stock: {(product.variants||[]).filter(v=>v.active).reduce((s,v)=>s+Number(v.stock||0),0)}</b></div><div className="mt-3 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">{(product.variants||[]).map(v=><label key={v.id} className="grid grid-cols-[1fr_100px] items-center gap-2 border p-3"><span>{v.size} · {v.color}</span><input className="field" type="number" min="0" value={v.stock} onChange={e=>setVariantStock(product.id,v.id,Number(e.target.value))}/></label>)}</div><button disabled={busy===product.id} onClick={()=>saveStock(product)} className="mt-3 bg-black px-4 py-3 text-xs font-black uppercase text-white"><Save size={15} className="mr-2 inline"/>Guardar stock</button></section>)}</div></div>}
